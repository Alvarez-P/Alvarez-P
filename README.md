<div align="center">

# Adolfo Esteban Álvarez Pacheco

### Senior Software Engineer

[![Location](https://img.shields.io/badge/Mérida-Yucatán,%20México-2ea44f?style=flat-square)](https://www.google.com/maps/place/Mérida,+Yucatán)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/adolfo-esteban-%C3%A1lvarez-pacheco-6b3a21192/)
[![Email](https://img.shields.io/badge/Email-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:adolfo_alvarez-p@outlook.es)

</div>

---

## 🧠 About Me

Ingeniero en Sistemas Computacionales con **6+ años de experiencia** especializado en **backend con Node.js, TypeScript y arquitecturas cloud sobre AWS**. Diseño soluciones escalables antes de escribir código e integro IA en mi flujo de desarrollo. He construido plataformas para telecomunicaciones, hotelería, logística, educación y servicios públicos.

→ 📄 **[Perfil completo — experiencia, formación y filosofía](./profile.md)**

---

## 🎯 Especialidades

| Arquitectura | Desarrollo | Calidad |
|:---:|:---:|:---:|
| Clean Architecture | APIs REST, GraphQL, WebSockets | Code Reviews |
| Domain-Driven Design (DDD) | Microservicios | Mentoría Técnica |
| Principios SOLID | Serverless (AWS Lambda) | AI-Assisted Development |
| Infraestructura como Código (AWS CDK) | Optimización de Rendimiento | Specification-Driven Development |

---

## 🛠 Tech Stack

<p align="center">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/typescript/typescript-original.svg" width="48" title="TypeScript"/>
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/nestjs/nestjs-original.svg" width="48" title="NestJS"/>
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/nodejs/nodejs-original.svg" width="48" title="Node.js"/>
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/react/react-original.svg" width="48" title="React"/>
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/nextjs/nextjs-original.svg" width="48" title="Next.js"/>
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/amazonwebservices/amazonwebservices-original-wordmark.svg" width="48" title="AWS"/>
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/postgresql/postgresql-original.svg" width="48" title="PostgreSQL"/>
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/mongodb/mongodb-original.svg" width="48" title="MongoDB"/>
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/docker/docker-original.svg" width="48" title="Docker"/>
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/jest/jest-plain.svg" width="48" title="Jest"/>
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/git/git-original.svg" width="48" title="Git"/>
</p>

<details>
<summary>📋 Stack completo</summary>

```yaml
Languages:     TypeScript, JavaScript, Python, Java, Go*
Backend:       NestJS, Express, HapiJS, Restify | REST, GraphQL, WebSockets
Frontend:      React, Next.js, Tailwind CSS, Zustand
Cloud (AWS):   Lambda, API Gateway, S3, SNS/SQS, Step Functions,
               EventBridge, EC2, VPC, RDS, IAM, AWS CDK
Databases:     PostgreSQL, PostGIS, MySQL | MongoDB, DynamoDB, Redis
ORMs:          Prisma, TypeORM, Sequelize, Drizzle, Mongoose
Messaging:     Kafka, RabbitMQ, SQS
DevOps:        Docker, Git, GitHub Actions, Bitbucket Pipelines, K8s*, Terraform*
Testing:       Jest, Supertest, Vitest, Mocha
AI Tooling:    Claude Code, OpenCode, Copilot, n8n, SDD
```

</details>

---

<div align="center">

### 📊 GitHub Stats
<img alt="Alvarez-P's GitHub Stats" src="https://github-readme-stats.vercel.app/api?username=Alvarez-P&show_icons=true&hide_border=true&theme=dark&count_private=true" />

---

### 🔗 Conecta conmigo

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/adolfo-esteban-%C3%A1lvarez-pacheco-6b3a21192/)
[![Email](https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:adolfo_alvarez-p@outlook.es)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Alvarez-P)

</div>
